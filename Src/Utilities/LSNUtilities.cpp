...
namespace lsn {
	...
	// == Functions.
	/**
	 * Gets the next UTF-16 character from a stream or error (LSN_UTF_INVALID).
	 * 
	...
	/**
	 * Gets the next UTF-8 character from a stream or error (LSN_UTF_INVALID).
	 * 
	...
	LSN_ERRORS Utilities::DownloadFile( const std::u16string &_pcUrl, const std::u16string &_pcPath ) {
		LSN_ERRORS eCode;
		{
			...
			/*try {
				...
				if ( !pPath.has_filename() ) { return LSN_E_INVALID_NAME; }
				if ( !std::filesystem::exists( std::filesystem::absolute( pPath.make_preferred().remove_filename() ) ) ) { return LSN_E_FOLDER_NOT_FOUND; }
			}
			catch ( ... ) { return LSN_E_FOLDER_NOT_FOUND; }*/


			LSN_CURL cCurl( ::curl_easy_init() );

			if ( !cCurl.Valid() ) { return LSN_E_CURL_INIT_FAILED; }
#define LSN_CHECK							if ( aCurlCode != CURLE_OK ) { return Errors::LibCurl_To_Native( aCurlCode ); }
			StdFile sfFile;
			...
			//::curl_easy_cleanup( cCurl.pcCurl );	// Called by LSN_CURL::~LSN_CURL().
		}
	...
	LSN_ERRORS Utilities::DownloadMnist( const std::u16string &_pcFolder ) {
		...
		try {
			...
			if ( !std::filesystem::exists( pPath ) ) { return LSN_E_FOLDER_NOT_FOUND; }
		...
		catch ( ... ) { return LSN_E_OUT_OF_MEMORY; }
